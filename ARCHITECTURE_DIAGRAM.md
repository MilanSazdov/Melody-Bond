# 📊 RWA Portfolio System Architecture Diagram

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                           FRONTEND APPLICATION                              │
└─────────────────────────────────────────────────────────────────────────────┘

┌──────────────────────┐  ┌──────────────────────┐  ┌──────────────────────┐
│   /portfolio Page    │  │  /governance Page    │  │   /projects Page     │
│                      │  │                      │  │                      │
│  • Show RWAs         │  │  • Show Proposals    │  │  • Fund RWAs         │
│  • Display Shares    │  │  • Weighted Voting   │  │  • Invest USDC       │
│  • TBA Balances      │  │  • Execute Proposals │  │  • Create Proposals  │
│  • Create Proposals  │  │  • Vote History      │  │                      │
└──────────┬───────────┘  └──────────┬───────────┘  └──────────┬───────────┘
           │                         │                         │
           │                         │                         │
           └─────────────┬───────────┴─────────────┬───────────┘
                         │                         │
                         ▼                         ▼
              ┌──────────────────────────────────────────┐
              │      lib/rwaGovernance.ts Library        │
              │                                          │
              │  • getUserRWAInvestments()               │
              │  • buildChangeNameProposal()             │
              │  • buildChangeImageProposal()            │
              │  • buildWithdrawProposal()               │
              │  • getRWAProposals()                     │
              │  • getUserVotingPower()                  │
              │  • getTBABalance()                       │
              └──────────────┬───────────────────────────┘
                             │
                             │ Web3 Calls
                             ▼

┌─────────────────────────────────────────────────────────────────────────────┐
│                          BLOCKCHAIN (SEPOLIA)                               │
└─────────────────────────────────────────────────────────────────────────────┘

┌────────────────────────────────────────────────────────────────────────────┐
│                              MAIN DAO CONTRACT                             │
│                    0x132AD6fB8EaF3065C831Febf5788AbDa4B72c76C              │
│                                                                            │
│  Storage:                                                                  │
│  • rwaShares[nftId][investor] = shares     ← VOTING POWER                  │
│  • rwaDaos[nftId] = governorAddress        ← RWA GOVERNOR                  │
│  • nftProposalId[nftId] = proposalId       ← FUNDING PROPOSAL              │
│  • rwaProposals[id] = {...}                ← PROPOSAL DATA                 │
│                                                                            │
│  Functions:                                                                │
│  • invest(proposalId, amount)              ← USER INVESTS USDC             │
│  • finalizeProposal(proposalId)            ← CREATES NFT + TBA             │
│  • rwaShares(nftId, investor)              ← READ VOTING POWER             │
│  • rwaDaos(nftId)                          ← READ GOVERNOR ADDR            │
└────────────────┬──────────────────────────────────────┬────────────────────┘
                 │                                      │
                 │ Creates & Tracks                     │ Reads Shares
                 ▼                                      ▼
    ┌────────────────────────┐         ┌───────────────────────────────┐
    │     RWA NFT CONTRACT   │         │   RWAGovernor (Per NFT)       │
    │      (ERC-721)         │         │         (Clone)               │
    │  0x914c81B1A6C3aCF7    │         │    One per RWA NFT            │
    │                        │         │                               │
    │  • tokenURI(nftId)     │         │  Storage:                     │
    │  • setTokenURI(...)    │◄────────┤  • nftId = X                  │
    │  • ownerOf(nftId)      │ Changes │  • tbaAddress = 0x...         │
    └────────────────────────┘  via    │  • mainDao = DAO address      │
                                Gov    │                               │
                                       │  Functions:                   │
                 ┌─────────────────────┤  • propose(...)               │
                 │                     │  • castVote(...)              │
                 │ Controls            │  • _getVotes(investor)        │
                 ▼                     │    → reads DAO.rwaShares      │
    ┌────────────────────────┐         └───────────┬───────────────────┘
    │   TBA Wallet (ERC-6551)│                     │
    │   Token Bound Account  │                     │ Executes
    │                        │                     │ Proposals
    │  • Holds USDC/ETH      │                     │
    │  • Owned by NFT        │                     ▼
    │  • Controlled by Gov   │         ┌────────────────────────────────┐
    │                        │         │  Proposal Execution            │
    │  executeCall(...)      │◄────────┤                                │
    └────────────┬───────────┘         │  1. Change Metadata:           │
                 │                     │     TBA → RWA.setTokenURI()    │
                 │                     │                                │
                 │ For Withdrawals     │  2. Withdraw Funds:            │
                 │                     │     TBA → USDC.approve()       │
                 │                     │     TBA → Distributor.dist..() │
                 ▼                     └────────────────────────────────┘
    ┌────────────────────────┐
    │  DISTRIBUTOR CONTRACT  │
    │  0xdD504aE23C6C63Ee    │
    │                        │
    │  distribute(           │
    │    nftId,              │
    │    amount,             │
    │    tokenAddress        │
    │  )                     │
    │                        │
    │  Logic:                │
    │  1. Pull from TBA      │
    │  2. Get investors list │
    │  3. Calculate shares % │
    │  4. Transfer to each   │
    └────────────┬───────────┘
                 │
                 │ Distributes
                 ▼
    ┌─────────────────────────────────────┐
    │        INVESTOR WALLETS             │
    │                                     │
    │  Alice (60% shares) → 600 USDC      │
    │  Bob   (40% shares) → 400 USDC      │
    │                                     │
    │  Proportional to rwaShares!         │
    └─────────────────────────────────────┘


┌─────────────────────────────────────────────────────────────────────────────┐
│                               FLOW DIAGRAMS                                 │
└─────────────────────────────────────────────────────────────────────────────┘

════════════════════════════════════════════════════════════════════════════
                           INVESTMENT FLOW
════════════════════════════════════════════════════════════════════════════

1. User invests USDC
   └─> DAO.invest(proposalId, amount)
       └─> Records: rwaShares[nftId][user] += shares

2. Proposal finalized
   └─> DAO.finalizeProposal(proposalId)
       ├─> Mints RWA NFT #X
       ├─> Creates TBA wallet for NFT #X
       ├─> Deploys RWAGovernor clone for NFT #X
       ├─> Records: rwaDaos[X] = governorAddress
       └─> Records: nftProposalId[X] = proposalId


════════════════════════════════════════════════════════════════════════════
                         PROPOSAL CREATION FLOW
════════════════════════════════════════════════════════════════════════════

1. User clicks "Propose" on Portfolio
   └─> Opens ProposalModal

2. User selects proposal type
   ├─> Change Name    → buildChangeNameProposal()
   ├─> Change Image   → buildChangeImageProposal()
   └─> Withdraw Funds → buildWithdrawProposal()

3. User fills form and submits
   └─> Calls: RWAGovernor.propose(targets, values, calldatas, description)
       └─> Creates proposal ID
       └─> Emits: ProposalCreated event


════════════════════════════════════════════════════════════════════════════
                            VOTING FLOW
════════════════════════════════════════════════════════════════════════════

1. User navigates to /governance
   └─> Frontend fetches: getRWAProposals(governorAddress, nftId)
       └─> Reads ProposalCreated events
       └─> For each proposal, reads state and votes

2. User sees proposal with their voting power
   └─> Frontend reads: DAO.rwaShares(nftId, userAddress)
       └─> Displays: "Your Voting Power: X shares"

3. User votes (For/Against/Abstain)
   └─> Calls: RWAGovernor.castVote(proposalId, support)
       ├─> Governor reads: DAO.rwaShares(nftId, voter)
       ├─> Records vote with share weight
       └─> Updates: proposalVotes[proposalId]

4. Vote distribution updates
   └─> Frontend reads: RWAGovernor.proposalVotes(proposalId)
       └─> Displays: For/Against/Abstain percentages


════════════════════════════════════════════════════════════════════════════
                         WITHDRAWAL FLOW (DETAILED)
════════════════════════════════════════════════════════════════════════════

STEP 1: Create Proposal
┌──────────────────────────────────────┐
│ buildWithdrawProposal(nftId, amount) │
│                                      │
│ Returns:                             │
│ targets = [USDC, Distributor]        │
│ values = [0, 0]                      │
│ calldatas = [                        │
│   approve(Distributor, amount),      │
│   distribute(nftId, amount, USDC)    │
│ ]                                    │
└──────────────────────────────────────┘

STEP 2: Voting
┌──────────────────────────────────────┐
│ Investors vote with share weight     │
│                                      │
│ Alice (600 shares) → FOR             │
│ Bob   (400 shares) → FOR             │
│                                      │
│ Total FOR: 1000 shares               │
│ Proposal SUCCEEDS                    │
└──────────────────────────────────────┘

STEP 3: Execution
┌──────────────────────────────────────┐
│ RWAGovernor._executeOperations()     │
│                                      │
│ Tells TBA to execute:                │
│ 1. USDC.approve(Distributor, 1000)   │
│ 2. Distributor.distribute(...)       │
└──────────────────────────────────────┘

STEP 4: Distribution
┌──────────────────────────────────────────────────────┐
│ Distributor.distribute(nftId, 1000, USDC)            │
│                                                      │
│ 1. transferFrom(TBA, Distributor, 1000)              │
│ 2. Get investors = DAO.getInvestorList(proposalId)   │
│ 3. totalShares = 1000 (Alice: 600, Bob: 400)         │
│ 4. For Alice:                                        │
│    payout = (1000 × 600) / 1000 = 600                │
│    USDC.transfer(Alice, 600) ✓                       │
│ 5. For Bob:                                          │
│    payout = (1000 × 400) / 1000 = 400                │
│    USDC.transfer(Bob, 400) ✓                         │
└──────────────────────────────────────────────────────┘

RESULT:
┌──────────────────────────────────────┐
│ TBA Balance:    1000 → 0 USDC        │
│ Alice Wallet:   +600 USDC            │
│ Bob Wallet:     +400 USDC            │
│                                      │
│ ✓ Proportional distribution!         │
└──────────────────────────────────────┘


════════════════════════════════════════════════════════════════════════════
                          KEY RELATIONSHIPS
════════════════════════════════════════════════════════════════════════════

SHARES → VOTING POWER → WITHDRAWAL %
   ↓          ↓              ↓
  60%    →   60%        →   60%

Example:
• Alice invests 600 USDC → 600 × 10^12 shares
• Total investment: 1000 USDC → 1000 × 10^12 total shares
• Alice's percentage: 600/1000 = 60%
• Alice's voting power: 60% of all votes
• Alice's withdrawal: 60% of all distributions

ONE UNIFIED SYSTEM! 🎯
```

This diagram shows the complete architecture and how everything connects!
